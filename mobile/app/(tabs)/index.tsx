...
import { Ship, Battery, Compass, Zap, ShieldAlert, CircleCheck, ChevronRight, PlaneTakeoff, PlaneLanding, Map } from 'lucide-react-native';
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.header}>
        ...
        <View style={[styles.statusBadge, connected ? styles.statusActive : styles.statusInactive]}>
          <ThemedText style={styles.statusText}>{connected ? 'LINK ACTIVE' : 'NO LINK'}</ThemedText>
        </View>
      </View>
      ...
      <View style={styles.telemetryGrid}>
        ...
        />
        <TelemetryCard
          icon={<Ship size={20} color="#3b82f6" />}
          label="POSITION"
      ...
      <ThemedView style={styles.opsPanel}>
        <ThemedText type="subtitle" style={styles.panelTitle}>FLIGHT OPERATIONS</ThemedText>
        ...
        <View style={styles.buttonRow}>
          ...
          >
            <PlaneLanding color="white" size={24} />
        ...
        {missionPlanned ? (
          <View style={styles.waypointList}>
            ...
            {waypoints.map((wp: Waypoint, i: number) => (
              <View key={wp.id} style={styles.waypointItem}>
                ...
                <ThemedText style={styles.wpAction}>{wp.action}</ThemedText>
                {data?.current_waypoint_id === wp.id && (
                  <CircleCheck
                    size={16}
                    color={i === 0 ? "#fde047" : "rgba(255,255,255,0.3)"}
                  />
                )}
              </View>
            ))}
            <TouchableOpacity onPress={startMission} style={styles.executeButton}>
              <ThemedText style={styles.executeButtonText}>EXECUTE MISSION</ThemedText>
          ...
          <View style={styles.placeholderBox}>
            ...
            <TouchableOpacity onPress={planMission} style={styles.planButton}>
              <ThemedText style={styles.planButtonText}>GENERATE HULL-ORBIT PLAN</ThemedText>
            </TouchableOpacity>
          </View>
        )}
      </ThemedView>
