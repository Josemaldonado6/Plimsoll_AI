...
def export_sam2(fp32_out: Path) -> Path:
    """Exporta SAM 2-B a OpenVINO IR FP32 con robustez total."""
    from ultralytics import SAM
    import torch
    
    logger.info("Exportando SAM 2-B (Arquitectura Dual) → OpenVINO FP32 ...")
    model = SAM("sam2_b.pt")
    
    # Extraemos el modelo PyTorch subyacente
    torch_model = model.model
    torch_model.eval()

    # Preparamos los inputs dummy según la arquitectura de SAM 2
    # SAM 2 usa un bloque de procesamiento de imagen
    img_dummy = torch.zeros(1, 3, *_SAM2_SIZE)
    
    try:
        # Intento 1: Conversión directa de la traza de Torch
        logger.info("Iniciando conversión de traza de Torch (ov.convert_model)...")
        ov_model = ov.convert_model(torch_model, example_input=img_dummy)
    except Exception as e:
        logger.warning(f"Falla en conversión directa: {e}. Intentando vía ONNX opset 17...")
        # Intento 2: ONNX como intermediario (más estable para modelos complejos)
        onnx_path = fp32_out / "temp_sam2.onnx"
        torch.onnx.export(
            torch_model, 
            img_dummy, 
            str(onnx_path),
    ...
    fp32_out.mkdir(parents=True, exist_ok=True)
    xml_path = fp32_out / "sam2_b.xml"
    ov.save_model(ov_model, str(xml_path))
    
    logger.info(f"✓ SAM 2-B FP32 IR Generado en: {xml_path}")
    return xml_path
