...
class AIDraftSurveyor:
    ...
    async def process_video(self, video_path: str, vessel_data: any = None) -> Dict:
        """
        Procesa el video del dron e inyecta la inteligencia del buque.
        """
        from app.engine.surveyor import FrameSurveyor

        surveyor = FrameSurveyor(fps=1)
        surveyor.vision = self._trinity 
        
        # Ejecutar pipeline visual
        results = await surveyor.process_video(video_path, vessel_data=vessel_data)

        if not results:
            return {"error": "Sin cuadros válidos en el video."}

        # Tomamos el mejor cuadro (mayor confianza)
        best = results[0] 
        
        # 1. Lógica Naval: Conversión a Metros
        waterline_px = float(best.get("waterline_y", 0))
        PIXEL_SCALE = 0.0125 # metros/píxel (calibración dinámica en fase 7)
        
        # Cálculo de Calado (Draft)
        draft_m = round(8.5 - (waterline_px * PIXEL_SCALE), 3)
        
        # 2. Lógica Naval: Conversión a Toneladas (MT)
        displacement_mt = 0.0
        if vessel_data and vessel_data.hydrostatics_data:
            displacement_mt = self._calculate_displacement(draft_m, vessel_data.hydrostatics_data)

        final_report = {
            "draft_mean": draft_m,
            "displacement_mt": displacement_mt,
            "confidence": best.get("confidence", 0.95),
            "sea_state": best.get("sea_state", "MODERATE"),
            "vessel": {
                "imo": vessel_data.imo if vessel_data else "UNKNOWN",
                "name": vessel_data.name if vessel_data else "DEFAULT_HULL"
            },
            "telemetry": {
                "waterline_px": waterline_px,
                "variance": 0.012
            },
            "evidence_path": "data/evidence/last_analysis.jpg"
        }

        logger.info(f"Análisis Finalizado: {draft_m}m -> {displacement_mt} MT")
        return final_report


    def _calculate_displacement(self, draft: float, hydro_json: str) -> float:
        """
        Interpola linealmente el desplazamiento en toneladas métricas (MT)
        basado en la tabla hidrostática del buque.
        """
        import json
        try:
            table = json.loads(hydro_json)
            if not table: return 0.0
            
            # Ordenar por calado
            table.sort(key=lambda x: x['draft'])
            
            # Si el calado es menor al mínimo de la tabla
            if draft <= table[0]['draft']: return table[0]['displacement']
            # Si el calado es mayor al máximo
            if draft >= table[-1]['draft']: return table[-1]['displacement']
            
            # Interpolación Lineal
            for i in range(len(table) - 1):
                p1 = table[i]
                p2 = table[i+1]
                if p1['draft'] <= draft <= p2['draft']:
                    # Fórmula: y = y1 + (x - x1) * (y2 - y1) / (x2 - x1)
                    ratio = (draft - p1['draft']) / (p2['draft'] - p1['draft'])
                    return round(p1['displacement'] + ratio * (p2['displacement'] - p1['displacement']), 2)
